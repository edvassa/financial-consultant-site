...
async function findAvailablePort(startPort: number = 3000): Promise<number> {
  ...
  async function startServer() {
    ...
    // PERMANENT FIX: Strip ?manus_scraper=1 parameter from ALL requests
    // This MUST be the FIRST middleware before any other processing
    // Manus proxy adds this parameter to all requests, but it breaks og:url
    app.use((req, res, next) => {
      if (req.query.manus_scraper) {
        delete req.query.manus_scraper;
        req.url = req.url.replace(/[?&]manus_scraper=1/g, '');
        req.originalUrl = req.originalUrl.replace(/[?&]manus_scraper=1/g, '');
        console.log('[STRIP] Removed ?manus_scraper=1 from request');
      }
      next();
    });
